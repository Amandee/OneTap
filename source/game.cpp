...
void Game::newGame()
{
	...
	AddChild(gemSprite2);
	gemSprite2->m_X = (float)IwGxGetScreenWidth() /1.2 ;
	gemSprite2->m_Y = (float)IwGxGetScreenHeight() / 2;
...
void Game::Update(float deltaTime, float alphaMul)
{
    ...
    // Detect screen tap
	if (m_IsInputActive && m_Manager->GetCurrent() == this && !g_pInput->m_Touched && g_pInput->m_PrevTouched && touched == false)
    {
		gemSprite->m_X = g_pInput->m_X;
		touched = true;
		g_pInput->Reset();
    }
	if (m_IsInputActive && m_Manager->GetCurrent() == this && !g_pInput->m_Touched && g_pInput->m_PrevTouched && touched == true)
	{
		touched = false;
		g_pInput->Reset();
	}
...
void Game::spriteMove()
{
	
	if (gemSprite->m_Y != 0)
	{
		
		gemSprite->m_Y = gemSprite->m_Y - 2;
		spriteCollision();
	}

}

void Game::spriteCollision()
{
	if (gemSprite->m_Y == gemSprite2->m_Y)
	{
		RemoveChild(gemSprite2);
...
void Game::Init()
{
	...
	AddChild(gemSprite);
	gemSprite2 = new CSprite();
	gemSprite2->m_X = (float)IwGxGetScreenWidth() / 2;
	gemSprite2->m_Y = (float)IwGxGetScreenHeight() / 2;
	gemSprite2->SetAtlas(g_pResources->getGemAtlas());
	gemSprite2->m_W = (float)gemSprite->GetAtlas()->GetFrameWidth();
	gemSprite2->m_H = (float)gemSprite->GetAtlas()->GetFrameHeight();
	gemSprite2->m_AnchorX = 0.5;
	gemSprite2->SetAnimDuration(2);
	AddChild(gemSprite2);
}
